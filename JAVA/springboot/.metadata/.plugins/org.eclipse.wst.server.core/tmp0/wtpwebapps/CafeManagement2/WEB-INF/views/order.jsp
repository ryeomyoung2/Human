...
<table style='width:100%; border:1px solid green;'>
	<tr><td style='width:33%; text-align:center'><a href='/menu'><h3>메뉴관리</h3></a></td>
		<td style='width:33%; text-align:center'><h3>주문관리</h3></a></td>
...
<tr>
    <td style='border:1px soild blue; vertical-align:top'>
    	<table>
	...
	<td style='vertical-align:top'>
		...
		<tr><td>모바일번호</td><td><input type=text id=mobile size=15>원</td></tr>
...
.on('click','#selMenu option',function(){   //1 
	let str=$(this).text();
	let ar=str.split(',');
	$('#name').val($.trim(ar[1]));
	$('#qty').val(1);
...
.on('change','#qty',function(){				//3
	let qty=parseInt($(this).val());
	let price=qty*parseInt($('#net').val());
	$('#price').val(price);
	return false;
})											//3
.on('click','#btnEmpty',function(){   		//4
	$('#name,#qty,#price').val('');
	return false;
})											//4
.on('click','#btnOrder',function(){			//5
	let str='<option>'+$('#name').val()+', x'+$('#qty').val()+', '+$('#price').val()+'</option>';
	$('#selOrder').append(str);
	$('#btnEmpty').trigger('click');		//5
	let total=0;                                //6
...
.on('click','#btnCancel',function(){			//7
	$('#selOrder').empty();
	$('#total,#mobile').val('');
	return false
})												//7
.on('click','#btnComplete',function(){		    //8	
	$('#selOrder option').each(function(){
		let str=$(this).text();
		let ar=str.split(',');
